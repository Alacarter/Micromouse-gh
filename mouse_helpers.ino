...
void turnRight() {
  ...
  Serial.println(readDistanceCenter());
  
  ticks_left = 0;
  ticks_right = 0;
  while (ticks_left - ticks_right < 260)
  {
  ...
  // clear integral term with sum errors
  // turn off PID: SetMode(Manual);
  // 285 encoder difference between L & R for 90 degree turn
  // turn on PID: SetMode (Automatic);
  // set angular velocity again.
}
...
void turnLeft() {
  ...
  ticks_left = 0;
  ticks_right = 0;
  while (ticks_right - ticks_left < 260)
  {
    Serial.println(ticks_right - ticks_left);
    applyPowerLeft(-0.2);
    applyPowerRight(0.2);
  }
  ticks_left = 0;
